...
import com.first.springweb.util.ProductValidator;
...
public class ProductController {
	...
	public String addProduct(@ModelAttribute("product") Product product, BindingResult result, Model model)
		...
		if (result.hasErrors()) {
			return "product";
		} else if (wareHouseService.productExists(new Long(product.getProductId()))) {
			result.rejectValue("productId", "product.already.exists");
			return "product";
		}

		ProductBO productBO = ProductUtil.covertToProductBO(product);
		int noRows = wareHouseService.addProduct(productBO); // layer
																// connectivity
		if (noRows > 0) {
